...
$(document).ready(function () {
    ...
    //lấy chuỗi fen nước đi
    hub.client.getFen = function (message) {
        createLog(message);
    }
...
function set_drop(id) {
    $('#' + id + '').droppable({
        ...
        drop: function (ev, ui) {
            ...
            hub.server.sendFen(param, fen_generate());
            console.log(fen_generate());
        }
    });
}

var init = 1;
function createLog(fen) {
    $('.move-table').append('<tr><td><p>' + init + '</p></td ><td><a class="fen_click">'+ fen + '</a></td>');
    init++;
    //bắt sự kiện nhấn fen lịch sử
    $(".fen_click").click(function () {
        var fens = $(this).text();
        Clear_board();
        hub.server.makeURfen(param, fens);
        draw_pieces(fens);
    })
}
